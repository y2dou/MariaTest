...
names.landuse = c("forest", "fallow", "garden", "acai", "other")
colors.landuse = c("#154229", "#CC723E", "#FF534C", "#54CC3E", "#99867B")
hashes.landuse = 
xlim = c(1,40)
...
econTrajectory = function(runID, householdID, xlim=NULL, ylim=c(0,500), ...) {
  ...
  #op = par(mfrow=c(3,2))
  op = par(no.readonly = TRUE, mar=c(4, 4, 2, 2) + 0.1, oma=c(0,0,0,0))
...
econTrajectoryPair = function ( runID1, runID2, householdID1, householdID2, composite=FALSE, singleTrajectory=TRUE, capital=TRUE, labour=TRUE, xlim=NULL, ylim1=NULL, ylim2=NULL, ylim3=NULL, capital.ylim=NULL, title1="", title2="", title3="", ...) {
  if (!(composite && singleTrajectory))
    return                 # must have one of these

  # prepare 3 charts: capital, labour and land trajectory
  op = par(no.readonly = TRUE, mar=c(0, 4, 0, 2) + 0.1, oma=c(1,0,3,0))
  ...
  d1 <- dbGetQuery(conn, "select tick, capital, labour, forest, fallow, maniocgarden, acai, other from tblHouseholdState where runID = ? and householdID = ?", runID1, householdID1)
  d2 <- dbGetQuery(conn, "select tick, capital, labour, forest, fallow, maniocgarden, acai, other from tblHouseholdState where runID = ? and householdID = ?", runID2, householdID2)
  ...
  if (singleTrajectory) {
    ...
    singleHouseholdAbsolute(runID2,householdID2,xlim=xlim,ylim=ylim2,xlab="tick",ylab="",main=ifelse(composite,'',title2),axes=FALSE)
    axis(2)
  ...
  if (capital) {
    if (is.null(capital.ylim))
      capital.ylim = c(0, max(d1$capital,d2$capital))
  
    plot(capital ~ tick, data = d1, xlim=xlim, ylim=capital.ylim, pch=19, cex=0.3, col="blue",xlab="tick", axes=FALSE)
    lines(capital ~ tick, data = d1, col="blue")
    axis(2)
    ...
    plot(capital ~ tick, data = d2, xlim=xlim, ylim=capital.ylim, ylab="", pch=19, cex=0.3, col="blue",xlab="tick", axes=FALSE)
    lines(capital ~ tick, data = d2, col="blue")
    axis(2)
  }

  if (labour) {
    plot(labour ~ tick, data = d1, xlim=xlim, ylim=c(0, max(d1$labour,d2$labour)), pch=19, cex=0.3, col="red", xlab="tick", ylab="labour availability",axes=FALSE)
    lines(labour ~ tick, data = d1, col="red")
    axis(2)
  }        
  title(xlab="tick")
  ...
  if (labour) {
    plot(labour ~ tick, data = d2, xlim=xlim, ylim=c(0, max(d1$labour,d2$labour)), pch=19, cex=0.3, col="red", xlab="tick", ylab="", axes=FALSE)
...
sensitivityScatterplotPair = function(sweepName, xName, yName, paramNames=c(), paramValues=c(), tick=40, xlim=NULL, ylim=NULL, title1=NULL, title2=NULL, title3=NULL, title4=NULL, xlab=NULL, ylab=NULL, cex=0.12, percent.of=NULL, xtransform = NULL, ytransform = NULL, use.lowess=FALSE, ...) {
  op = par(no.readonly = TRUE, mar=c(4.5, 4, 3, 2) + 0.1, oma=c(0,0,0,0))
  
  # set 2 chart layout
  new.layout <- layout(matrix(c(1,2,3,4), 1, 4, byrow=TRUE))
  
  paramNames1=c(paramNames,"percentOptimalHouseholds")  
  paramNames2=c(paramNames,"percentForwardOptimalHouseholds")
  paramNames3=c(paramNames,"percentFullForwardOptimalHouseholds")
  paramNames4=c(paramNames,"percentHeuristicHouseholds")
  ...
  d3 <- sensitivityQuery(sweepName,xName,yName,tick=tick,paramNames3,paramValues)
  d4 <- sensitivityQuery(sweepName,xName,yName,tick=tick,paramNames4,paramValues)
  
  if (is.null(ylim))
    ylim = c(min(d1[,yName],d2[,yName],d3[,yName],d4[,yName]),max(d1[,yName],d2[,yName],d3[,yName],d4[,yName]))
  ...
  sensitivityScatterplot(sweepName,xName,yName,tick=tick,paramNames=paramNames3,paramValues=paramValues,d=d3,percent.of=percent.of,xlim=xlim,ylim=ylim,xlab=xlab,ylab=ylab,cex=cex,xtransform=xtransform,ytransform=ytransform,use.lowess=use.lowess,main=title3, ...)
  sensitivityScatterplot(sweepName,xName,yName,tick=tick,paramNames=paramNames4,paramValues=paramValues,d=d4,percent.of=percent.of,xlim=xlim,ylim=ylim,xlab=xlab,ylab=ylab,cex=cex,xtransform=xtransform,ytransform=ytransform,use.lowess=use.lowess,main=title4, ...)
...
sensitivityBoxplotPair = function(sweepName, xName, yName, paramNames=c(), paramValues=c(), tick=40, xlim=NULL, ylim=NULL, xlab=NULL, ylab=NULL, title1=NULL, title2=NULL, cex=0.12, percent.of=NULL, xtransform = NULL, ytransform = NULL, use.lowess=FALSE, ...) {
  ...
  # set 2 chart layout
  new.layout <- layout(matrix(c(1,2,3), 1, 3, byrow=TRUE))
  
  paramNames1=c(paramNames,"percentOptimalHouseholds") 
  paramNames2=c(paramNames,"percentForwardOptimalHouseholds") 
  paramNames3=c(paramNames,"percentHeuristicHouseholds")
  paramValues=c(paramValues,1)
  ...
  if (is.null(ylim))
    ylim = c(min(d1[,yName],d2[,yName],d3[,yName]),max(d1[,yName],d2[,yName],d3[,yName]))
  ...
  sensitivityBoxplot(sweepName,xName,yName,tick=tick,paramNames=paramNames3,paramValues=paramValues,d=d3,percent.of=percent.of,xlim=xlim,ylim=ylim,xlab=xlab,ylab=ylab,xtransform=xtransform,ytransform=ytransform,use.lowess=use.lowess,main=title2, ...)
...
singleHousehold = function (runID, householdID, xlim=c(0,1), ylim=c(0,1), xlab="tick", ylab="cells", ...) {
  d <- dbGetQuery(conn, "select tick, capital, labour, forest, fallow, maniocgarden, acai, other from tblHouseholdState where runID = ? and householdID = ?", runID, householdID)
...
linkedHouseholdsNumComposite = function (runID, xlab="tick", xlim=c(0,40), ylim=NULL, lwd.base=0.3, cols=NULL, ...) {
  ...
  plot(c(),c(),xlim=xlim,ylim=ylim,xlab=xlab,ylab="# of off-site agents",axes=FALSE,...)
  axis(1)
  axis(2)
